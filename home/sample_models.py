...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for i in range(0, recur_layers):
        deep_rnn = GRU(units, activation='relu', return_sequences=True,
                       implementation=2, name='rnn'+str(i))(bn_rnn)
        bn_rnn = BatchNormalization(name='bn_rnn'+str(i))(deep_rnn)
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim), name='time_dense')(bn_rnn)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
...
def bidirectional_rnn_model(input_dim, units, output_dim=29):
    ...
    """
    # Main acoustic input
    input_data = Input(name='the_input', shape=(None, input_data))
...
def final_model():
    ...
    bn_rnn = bn_cnn
    for i in range(0, recur_layers):
        deep_rnn = CuDNNGRU(units, return_sequences=True,
                       name='rnn'+str(i))(bn_rnn)
        dropout_rnn = Dropout(0.3)(deep_rnn)
        bn_rnn = BatchNormalization(name='bn_rnn'+str(i))(dropout_rnn)
